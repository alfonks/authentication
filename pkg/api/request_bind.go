...
func pathParamToMap(c echo.Context) map[string][]string {
	param := make(map[string][]string)
	paramVal := c.ParamValues()
	paramName := c.ParamNames()

	for i := range paramVal {
		param[paramName[i]] = []string{paramVal[i]}
	}

	return param
}

func Bind(c echo.Context, i interface{}) (err error) {
	r := c.Request()

	params := pathParamToMap(c)
	...
	default:
		return fmt.Errorf(http.StatusText(http.StatusUnsupportedMediaType))
	}
	return
}
